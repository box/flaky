...
.. code-block:: python
    ...
    def test_something_that_usually_passes(self):
        ...
        assert result == value_to_double * 2, 'Result doubled incorrectly.'

By default, flaky will retry a failing test once, but that behavior can be overridden by passing values to the
flaky decorator. It accepts ``max_runs``, and ``min_passes``; flaky will run tests up to ``max_runs`` times, until
it has succeeded ``min_passes`` times. Once a test passes ``min_passes`` times, it's considered a success;
once it has been run max_runs times without passing min_passes times, it's considered a failure.
...
.. code-block:: python
    ...
    class TestMultipliers:
        ...
        def test_flaky_tripler(self):
            ...
            assert result == value_to_triple * 3, 'Result tripled incorrectly.'
...
``@pytest.mark.flaky`` can be used in place of ``@flaky``.
...
Flaky can be disabled via the command line:
...
Works with ``pytest-xdist`` but not with the ``--boxed`` option. Doctests cannot be marked flaky.
